            ...
            <nav className="border-b-2 border-foreground fixed top-0 left-0 right-0 bg-background/95 backdrop-blur-sm font-header z-20">
                <Container>
                    ...
                    <div className="flex items-center">
                        ...
                        <Newspaper />
                        <h1 className="uppercase font-bold font-display text-lg">The Portfolio</h1>
                        </div>
                    </div>
                    <div onClick={handleMenuMobile} className="p-2 border-2 border-foreground">
                        {menuMobileOpen ? <X /> : <Menu />}
                    </div>
                    </div>
                </Container>
            </nav>
            <div className={`fixed top-0 left-0 right-0 bottom-0 bg-background pt-19 border-l-2 border-foreground z-10 duration-400 ${menuMobileOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <MenuContent />
            </div>
        </>
    );
